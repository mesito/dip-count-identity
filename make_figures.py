...
def fig2():
    ...
    plt.title("Normalised floors: zeta (solid) against CUE (dashed)"); save("fig2_floor_cdf")
    ...
    plt.title("The shallow tail is suppressed arithmetically"); plt.legend(fontsize=7); save("fig3_tail_ratio")
...
def fig6():
    ...
    plt.xlabel(r"$t_0$"); plt.ylabel(r"$h_c\log(t_0/2\pi)$"); plt.legend(fontsize=7); plt.title("Transition depth and the half-gap threshold"); save("fig6_transition")
...
def fig8():
    ...
    plt.xlabel("test point (E3, $t\\approx3.06\\times10^{10}$)"); plt.ylabel("|error| vs mpmath"); plt.legend(fontsize=7); plt.title("Evaluator error"); save("fig8_evaluator")


# ------------------------------------------------------------------ fig9: T-invariance bars
def fig9():
    lab = ["E1", "E2", "E3"]; C = [0.53, 0.40, 0.43]; al = [0.39, 0.42, 0.52]; co = [-0.598, -0.583, -0.583]; sl = [-0.742, -0.860, -0.858]
    fig, ax = plt.subplots(1, 4, figsize=(8, 2.4)); cols = ["tab:brown", "tab:green", "tab:purple"]
    for a, v, t in zip(ax, (C, al, co, sl), (r"amplitude $C$", r"exponent $\alpha$", r"corr(tilt, depth)", r"slope $\log$ratio $\sim F$")):
        a.bar(lab, v, color=cols); a.set_title(t, fontsize=8)
        for i, vv in enumerate(v): a.text(i, vv + (0.02 if vv > 0 else -0.06), "%.3f" % vv, ha="center", fontsize=7)
    ax[1].axhline(0.5, color="k", ls="--", lw=0.6); save("fig9_tinvariance")
...
def fig10():
    ...
    save("fig10_identities")


if __name__ == "__main__":
    which = sys.argv[1:] or ["fig1", "fig2", "fig4", "fig5", "fig6", "fig7", "fig8", "fig9", "fig10"]
    for w in which:
